    ...
    VStack(spacing: 20) {
        ...
        VStack(spacing: 12) {
            HStack {
                ...
                Spacer()
                HStack {
                    Image(systemName: TradeDirection.buy.icon)
                    Text(TradeDirection.buy.rawValue)
                }
                .foregroundColor(TradeDirection.buy.color)
            }
            
            HStack {
                Text("Sample Bot:")
                Spacer()
                if let bot = TradingBot.sampleBots.first {
                    Text(bot.name)
                        .fontWeight(.semibold)
            ...
            HStack {
                Text("Gold Price:")
                Spacer()
                Text(SampleData.goldPrice.formattedPrice)
                    .font(.headline)
